...
export const ChecklistProgress = ({ currentStepIndex }: Props) => {
  return (
    <div className="bg-white dark:bg-background-dark shadow-md rounded-xl p-6">
      ...
      <div className="mt-4 h-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
        <motion.div
          ...
          style={{
            width: `${((currentStepIndex + 1) / journeySteps.length) * 100}%`,
          }}
          initial={{ width: 0 }}
          animate={{
            width: `${((currentStepIndex + 1) / journeySteps.length) * 100}%`,
